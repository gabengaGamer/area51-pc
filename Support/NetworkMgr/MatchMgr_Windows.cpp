//==============================================================================
//
// MatchMgr_Windows.cpp - Matchup manager interface functions.
//
//==============================================================================
//
...
void s_MatchPeriodicUpdater( void* pData )
{
    match_mgr& MatchMgr = *(match_mgr*)pData;
    xtimer deltatime;

    deltatime.Start();

    while( !MatchMgr.m_ForceShutdown )
    {
...
xbool match_mgr::Init( net_socket& Local, const net_address Broadcast )
{
    ...
    ASSERTS( x_WorkersIsInit(), "MatchMgr requires x_workers to be initialized" );
    if( !x_WorkersIsInit() || !x_WorkerServiceStart( s_MatchPeriodicUpdater, this, "MatchMgr", m_WorkerService ) )
    {
        ASSERTS( FALSE, "MatchMgr failed to start worker service" );
        m_Initialized = FALSE;
        UnlockLists();
        return FALSE;
    }
...
void  match_mgr::Kill                ( void )
{
    if( m_Initialized )
    {
        ...
        xtimer t;
        t.Start();
        while( m_ForceShutdown && m_WorkerService.IsNonNull() && !x_WorkerServiceIsDone( m_WorkerService ) )
        {
            x_DelayThread( 2 );
            if( t.ReadMs() > 3000.0f )
            {
...
void match_mgr::SetState( match_mgr_state NewState )
{
    ...
    case MATCH_GET_MESSAGES:
        ...
        //{
        //    m_MessageOfTheDay.Clear();
        //    x_sprintf(m_DownloadFilename,"%s/%s_%d_Message.txt",MATCH_DOWNLOAD_LOCATION, x_GetLocaleString(), g_Changelist );
        //    InitDownload( m_DownloadFilename );
        //}
        // Skip MOTD download
        m_MessageOfTheDayReceived = TRUE;
