...
#define _PC_RENDER_H_
    ...
    struct render_target //So, do we really need this ? Probably the better way is call DX9 API directly
    {
        bool SetFromTextures( IDirect3DTexture9* pTarget, IDirect3DTexture9* pDepth ) //SetTile !! will be here
        {
            ...
            pTarget->GetSurfaceLevel( 0, &pTargetSurface );
            pDepth->GetSurfaceLevel( 0, &pDepthSurface );
            
            bool result = Set( pTargetSurface, pDepthSurface );
        ...
        bool Set                   ( IDirect3DSurface9* pTarget, IDirect3DSurface9* pDepth );
        bool Set                   ( s32 Target, s32 Depth );  //SetTiled !D! will be here
    
        IDirect3DSurface9* GetTarget( void )
        {
            return m_pTarget[m_Stack];
        }
    
        IDirect3DSurface9* GetDepth( void )
        {
    ...
    private:
        ...
        IDirect3DSurface9* m_pTarget[4];
        IDirect3DSurface9* m_pDepth [4];
