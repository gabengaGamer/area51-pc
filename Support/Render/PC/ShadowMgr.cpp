...
shadow_mgr::shadow_mgr( void ) :
    ...
    m_CurrentSource            ( -1 ),
    m_pPointShadowTexture      ( NULL ),
    m_pPointShadowSRV          ( NULL ),
    ...
    m_ShadowLightBleedReduction( 0.20f )
{
    ...
    x_memset( m_pPointShadowDSV, 0, sizeof(m_pPointShadowDSV) );
}
...
void shadow_mgr::Kill( void )
{
    ...
    for( s32 i = 0; i < ARRAYSIZE(m_pPointShadowDSV); i++ )
        ReleaseCOM( m_pPointShadowDSV[i] );

    ReleaseCOM( m_pPointShadowSRV );
    ReleaseCOM( m_pPointShadowTexture );
...
void shadow_mgr::EnsureAtlas( void )
{
    ...
    ColorReg.BaseWidth      = SHADOW_ATLAS_SIZE;
    ColorReg.BaseHeight     = SHADOW_ATLAS_SIZE;
    ...
    DepthReg.BaseWidth      = SHADOW_ATLAS_SIZE;
    DepthReg.BaseHeight     = SHADOW_ATLAS_SIZE;
...
{
    if( m_pPointShadowTexture && m_pPointShadowSRV )
        return;
    ...
    Desc.Width              = POINT_SHADOW_FACE_SIZE;
    Desc.Height             = POINT_SHADOW_FACE_SIZE;
    Desc.MipLevels          = 1;
    Desc.ArraySize          = MAX_SHADOW_LIGHTS * POINT_SHADOW_FACE_COUNT;
    ...
    HRESULT hr = g_pd3dDevice->CreateTexture2D( &Desc, NULL, &m_pPointShadowTexture );
    if( FAILED(hr) )
    {
        x_DebugMsg( "ShadowMgr: failed to create point shadow cube array (HRESULT 0x%08X)\n", hr );
        return;
    }
    ...
    SRVDesc.TextureCubeArray.NumCubes         = MAX_SHADOW_LIGHTS;

    hr = g_pd3dDevice->CreateShaderResourceView( m_pPointShadowTexture, &SRVDesc, &m_pPointShadowSRV );
    if( FAILED(hr) )
    {
        x_DebugMsg( "ShadowMgr: failed to create point shadow cube SRV (HRESULT 0x%08X)\n", hr );
        ReleaseCOM( m_pPointShadowTexture );
        return;
    }

    for( s32 iFace = 0; iFace < ARRAYSIZE(m_pPointShadowDSV); iFace++ )
    {
        ...
        DSVDesc.Texture2DArray.FirstArraySlice = iFace;
        DSVDesc.Texture2DArray.ArraySize       = 1;

        hr = g_pd3dDevice->CreateDepthStencilView( m_pPointShadowTexture,
                                                   &DSVDesc,
                                                   &m_pPointShadowDSV[iFace] );
        if( FAILED(hr) )
        {
            x_DebugMsg( "ShadowMgr: failed to create point shadow face DSV %d (HRESULT 0x%08X)\n", iFace, hr );
            for( s32 iRelease = 0; iRelease <= iFace; iRelease++ )
                ReleaseCOM( m_pPointShadowDSV[iRelease] );
            ReleaseCOM( m_pPointShadowSRV );
            ReleaseCOM( m_pPointShadowTexture );
            return;
        }
...
void shadow_mgr::ApplySource( s32 SourceIndex )
{
    ...
    if( Source.Type == shadow_map_mgr::SHADOW_SOURCE_POINT_FACE )
    {
        ...
        const s32 SliceIndex    = Source.PointLightIndex * POINT_SHADOW_FACE_COUNT + CubeFaceIndex;
        if( ( SliceIndex < 0 ) || ( SliceIndex >= ARRAYSIZE(m_pPointShadowDSV) ) || !m_pPointShadowDSV[SliceIndex] )
            return;

        Viewport.TopLeftX = 0.0f;
        Viewport.TopLeftY = 0.0f;
        Viewport.Width    = (FLOAT)POINT_SHADOW_FACE_SIZE;
        Viewport.Height   = (FLOAT)POINT_SHADOW_FACE_SIZE;
        g_pd3dContext->RSSetViewports( 1, &Viewport );

        g_pd3dContext->OMSetRenderTargets( 0, NULL, m_pPointShadowDSV[SliceIndex] );
        if( !m_SourceCleared[SourceIndex] )
        {
            g_pd3dContext->ClearDepthStencilView( m_pPointShadowDSV[SliceIndex], D3D11_CLEAR_DEPTH, 1.0f, 0 );
...
void shadow_mgr::RenderSkinCaster( xhandle hDList, const matrix4* pBones, s32 SourceIndex )
{
    ...
    g_SkinVertMgr.DrawDList( hDList, pBones, NULL );
}
...
ID3D11ShaderResourceView* shadow_mgr::GetPointShadowSRV( void ) const
{
    return m_pPointShadowSRV;
}
...
{
    return 1.0f / (f32)SHADOW_ATLAS_SIZE;
}
