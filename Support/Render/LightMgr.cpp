...
static
f32 ComputeLightScore( const xcolor& Color, f32 Intensity )
{
    const f32 R = (f32)Color.R * Intensity;
    const f32 G = (f32)Color.G * Intensity;
    const f32 B = (f32)Color.B * Intensity;

    return R*R + G*G + B*B;
}

static
void ScaleLightColor( xcolor& Dst, const xcolor& Src, f32 Intensity )
{
    Dst.R = (u8)MIN( 255.0f, (f32)Src.R * Intensity );
    Dst.G = (u8)MIN( 255.0f, (f32)Src.G * Intensity );
    Dst.B = (u8)MIN( 255.0f, (f32)Src.B * Intensity );
    Dst.A = (u8)MIN( 255.0f, (f32)Src.A * Intensity );
}
                                ...
                                const vector3& TargetPos )
{
    ...
    const f32 SpotCos = LightDirection.Dot( ToTarget );
                                  ...
                                  f32            ConeLength )
{
    ...
    const f32     AxialDistance  = ConeAxis.Dot( ToCenter );
    ...
    {
        const f32 SideDistance = ( RadialDistance * ConeOuterCos ) - ( AxialDistance * ConeOuterSin );
        if( SideDistance <= SphereRadius )
            return TRUE;
    }

    const f32 ConeCapRadius  = ConeLength * ConeOuterTan;
...
static
xbool SpotLightAffectsSphere( const vector3& LightPos,
                              const vector3& LightDirection,
                              f32            OuterConeCos,
                              f32            OuterConeSin,
                              f32            OuterConeTan,
                              ...
                              f32            SphereRadius )
{
    ...
    return SphereIntersectsFiniteCone( SphereCenter,
                                       SphereRadius,
                                       LightPos,
                                       LightDirection,
                                       OuterConeCos,
...
static
void SetupLightCone( f32  InnerAngle,
                     f32  OuterAngle,
                     f32& InnerConeCos,
                     f32& OuterConeCos,
                     f32& OuterConeSin,
                     ...
                     f32& ConeCosRangeInv )
{
    ...
    const f32 ConeCosRange = MAX( InnerConeCos - OuterConeCos, 0.0001f );
...
static
void SetupOmniLightCone( f32& InnerConeCos,
                         f32& OuterConeCos,
                         f32& OuterConeSin,
                         f32& OuterConeTan,
                         f32& ConeCosRangeInv )
                                 ...
                                 const texture::handle& Cookie )
{
    ...
    else
    {
        ...
        {
            SetupOmniLightCone( InnerConeCos,
                                OuterConeCos,
                                OuterConeSin,
                                OuterConeTan,
                                ConeCosRangeInv );
...
s32 light_mgr::AddLight( void )
{
    ...
    for ( iLight = 0; iLight < MAX_FADING_LIGHTS; iLight++ )
    {
        ...
        f32 I = ComputeLightScore( m_FadingLights[iLight].CurrentColor,
                                   m_FadingLights[iLight].Intensity );
...
void light_mgr::InsertCollectedCharLight( const dir_light& Light, s32 MaxLightCount )
{
    ...
    const f32 NewScore = ComputeLightScore( Light.Col, 1.0f );
    s32 InsertAt = m_NCollectedLights;

    if( m_NCollectedLights == MaxLightCount )
    {
        const f32 LowestScore = ComputeLightScore( m_CollectedCharLights[m_NCollectedLights-1].Col, 1.0f );
    ...
    while( InsertAt > 0 )
    {
        const f32 PrevScore = ComputeLightScore( m_CollectedCharLights[InsertAt-1].Col, 1.0f );
...
void light_mgr::BeginLightCollection( void )
{
    ...
    if( nLights > 0 )
        m_pSpadLights = (spad_light*)smem_StackAlloc( sizeof(spad_light)*nLights );
    else
        m_pSpadLights = NULL;
...
s32 light_mgr::CollectLights( const bbox& WorldBBox, s32 MaxLightCount )
{
    ...
    const vector3 WorldBBoxCenter = WorldBBox.GetCenter();
    const f32     WorldBBoxRadius = WorldBBox.GetRadius();
    ...
    for ( i = 0; (i < m_nNonCharLightsInSpad) && (m_NCollectedLights < MaxLightCount); i++ )
    {
        ...
        {
            bIntersects = SpotLightAffectsSphere( m_pSpadLights[i].Pos,
                                                  m_pSpadLights[i].Direction,
                                                  m_pSpadLights[i].OuterConeCos,
                                                  m_pSpadLights[i].OuterConeSin,
                                                  m_pSpadLights[i].OuterConeTan,
                                                  m_pSpadLights[i].Radius,
                                                  WorldBBoxCenter,
                                                  WorldBBoxRadius );
        }
...
s32 light_mgr::CollectCharLights( const matrix4& L2W, const bbox& B, s32 MaxLightCount )
{
    ...
    const vector3 WorldBoxCenter = L2W * B.GetCenter();
    dir_light Light;
    ...
    for ( s32 iDynamicLight = 0; iDynamicLight < m_NDynamicLights; iDynamicLight++ )
    {
        if ( CalcDirLight( &Light,
                           ...
                           m_DynamicLights[iDynamicLight].OuterConeCos,
