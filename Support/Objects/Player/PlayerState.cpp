...
static const f32 s_CinemaViewCorrectionTime     = 0.95f;
...
void player::BeginState( void )
{
    ...
    case ANIM_STATE_CINEMA:             /* Reserved legacy slot. */ break;
...
void player::UpdateDeath( const f32& DeltaTime )
{
    ...
    xbool PrimaryPressed = m_Input.GetState().WasPressed( PlayerAction::PrimaryFire );
...
void player::UpdateIdle( const f32& DeltaTime )
{
    ...
    if( !m_bHidePlayerArms )
    {
        ...
        xbool bPressedReload   = !bUsedFocusObject && m_Input.GetState().WasPressed( PlayerAction::Reload );
...
void player::UpdateFire( const f32& DeltaTime )
{
    ...
    m_LastTimeWeaponFired = (f32)g_ObjMgr.GetSimulationTimeSeconds();
...
void player::UpdateFallingToDeath( f32 DeltaTime )
{
    ...
    if( m_DeathCamera.IsActive() )
    {
        m_DeathCamera.SetDesiredPitch( -m_Physics.GetVelocity().GetPitch() );
    }
}
...
void player::UpdateZoomIn( const f32& DeltaTime )
{
    ...
    if( !IsAimToggleEnabled() && !IsAltFiring() )
    {
        animation_state const NextAnimState = GetMotionTransitionAnimState();
        SetAnimState( ANIM_STATE_ZOOM_OUT );
        m_NextAnimState = NextAnimState;
        return;
    }
...
void player::UpdateZoomIdle( const f32& DeltaTime )
{
    ...
    xbool const AimToggleEnabled = IsAimToggleEnabled();
    ...
    else if( !AimToggleEnabled && !IsAltFiring() )
    {
        SetAnimState( ANIM_STATE_ZOOM_OUT );
        m_NextAnimState = ANIM_STATE_IDLE;
        return;
    }
    else if( AimToggleEnabled && IsAltFirePressed() )
    {
...
void player::UpdateZoomRun( const f32& DeltaTime )
{
    ...
    {
        SetAnimState( ANIM_STATE_ZOOM_OUT );
        m_NextAnimState = ANIM_STATE_RUN;
        return;
    }
    else if( AimToggleEnabled && IsAltFirePressed() )
    {
...
//==============================================================================
//  ORTHOGONAL PLAYER CINEMA MODE
//==============================================================================
void player::BeginCinema( void )
{
    if( m_CinemaController.IsActive() )
    {
        m_CinemaController.UpdateSettings( m_CinemaSettings );
        return;
    }

    m_CinemaController.Begin( m_CinemaSettings );
    ...
    m_CinemaSolveActorCollisions = m_Physics.IsSolvingActorCollisions();
    m_CinemaLocoCollision        = m_Physics.IsLocoCollisionOn();
    m_CinemaLocoGravity          = m_Physics.IsLocoGravityOn();
    m_CinemaPhysicsCaptured      = TRUE;
...
void player::UpdateCinema( f32 DeltaTime )
{
    ...
    object* pCameraObject = NULL;
    object_ptr<camera> pCamera( m_CinemaSettings.CameraGuid );
    if( m_CinemaSettings.CameraGuid != 0 )
    {
        pCameraObject = g_ObjMgr.GetObjectByGuid( m_CinemaSettings.CameraGuid );
        if( !pCameraObject )
        {
            EndCinema( PlayerCinemaFinishReason::Error );
            return;
        }
    ...
    vector3 EyePosition;
    radian3 SourceRotation( 0.0f, 0.0f, 0.0f );
    if( pCameraObject )
    {
        m_CinemaViewSample = PlayerViewSample();
        m_CinemaViewSample.Mode = PlayerViewMode::Cinema;
        m_CinemaViewSample.ViewToWorld = pCameraObject->GetL2W();
        m_CinemaViewSample.XFOV = m_ViewInfo.XFOV;

        if( pCamera )
        {
            zone_mgr::tracker const CameraZoneTracker = pCamera->GetZoneTracker();
            m_CinemaViewSample.HasZoneSnapshot = TRUE;
            m_CinemaViewSample.Zone1 = CameraZoneTracker.GetMainZone();
            m_CinemaViewSample.Zone2 = CameraZoneTracker.GetZone2();
        }

        if( pCamera )
        {
            view const& CameraView = pCamera->GetView();
            m_CinemaViewSample.XFOV = CameraView.GetXFOV();
            CameraView.GetZLimits( m_CinemaViewSample.NearClip,
                                   m_CinemaViewSample.FarClip );
        }

        EyePosition    = m_CinemaViewSample.ViewToWorld.GetTranslation();
        SourceRotation = m_CinemaViewSample.ViewToWorld.GetRotation();
    }
    else
    {
        EyePosition = m_SimulationViewInitialized
                    ? GetEyesPosition()
                    : GetDefaultViewPos();
        SourceRotation.Set( m_Pitch, m_Yaw, 0.0f );
    }

    object* pLookAt = g_ObjMgr.GetObjectByGuid( m_CinemaSettings.LookAtTargetGuid );
    if( !pLookAt )
    {
        return;
    }

    vector3 CurrentDirection( 0.0f, 0.0f, 1.0f );
    CurrentDirection.RotateX( SourceRotation.Pitch );
    CurrentDirection.RotateY( SourceRotation.Yaw );

    m_CinemaController.Advance( DeltaTime );
    f32 const BlendT = m_CinemaController.GetLookAtBlend();

    radian Pitch;
    radian Yaw;
    if( !PlayerCinema::BlendLookAt( CurrentDirection,
                                    EyePosition,
                                    pLookAt->GetPosition(),
                                    BlendT,
                                    Pitch,
    ...
    if( pCameraObject )
    {
        SourceRotation.Pitch = Pitch;
        SourceRotation.Yaw   = Yaw;
        m_CinemaViewSample.ViewToWorld.Setup( vector3( 1.0f, 1.0f, 1.0f ),
                                               SourceRotation,
                                               EyePosition );
    }
    else
...
void player::EndCinema( PlayerCinemaFinishReason Reason )
{
    if( !m_CinemaController.IsActive() )
    {
        return;
    ...
    PlayerViewSample const LastCinemaView = m_ViewController.GetCurrent();
    xbool const UseViewCorrection = m_CinemaSettings.UseViewCorrection;
    m_CinemaController.End( Reason );
    if( UseViewCorrection )
    {
        m_ViewController.BeginBlend( LastCinemaView, s_CinemaViewCorrectionTime );
    }

    hud_object* Hud = GetHud();
    if( m_CinemaPhysicsCaptured )
    {
        m_Physics.SetSolveActorCollisions( m_CinemaSolveActorCollisions );
        m_Physics.SetLocoCollisionOn( m_CinemaLocoCollision );
        m_Physics.SetLocoGravityOn( m_CinemaLocoGravity );
...
static const f32 s_MissionFailedDisplayTime = 3.0f;
static const f32 s_MissionFailedFadeOutTime = 2.0f;

void player::UpdateMissionFailed( f32 DeltaTime )
{
    const f32 LastTimeInState = m_TimeInState - DeltaTime;
    if ( IN_RANGE( LastTimeInState, s_MissionFailedDisplayTime, m_TimeInState ) )
    {
        g_PostEffectMgr.StartScreenFade( xcolor(0,0,0,255), s_MissionFailedFadeOutTime );
    }
    else if ( IN_RANGE( LastTimeInState,
                        s_MissionFailedDisplayTime + s_MissionFailedFadeOutTime,
                        m_TimeInState ) )
    {
#if !defined( X_EDITOR )
