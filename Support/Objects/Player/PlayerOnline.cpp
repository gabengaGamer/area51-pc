...
void player::OnGameSpeak( void )
{
    ...
    if( m_Input.GetState().WasPressed( PlayerAction::SpeakFollowStay ) )
    {
        ...
        if( m_Input.GetState().WasPressed( PlayerAction::SpeakUseActivate ) )
        {
        ...
        if( m_Input.GetState().WasPressed( PlayerAction::SpeakCoverMe ) )
            {
                // Just make sure we didn't get here without having someone to talk to.
                ASSERT( pCharacter ) ;
        ...
        if( m_Input.GetState().WasPressed( PlayerAction::SpeakAttackCover ) )
                {
                    return ;
                }
                else
