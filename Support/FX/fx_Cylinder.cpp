...
{
    X_PROFILE_SCOPE_CATEGORY( "Context", "fx_cylinder::AdvanceLogic" );
...
void fx_cylinder::SubmitRender( const fx_effect_base* pEffect ) const
{
    X_PROFILE_SCOPE_CATEGORY( "Context", "fx_cylinder::SubmitRender" );
    ...
    const texture* pDiffuse = pEffect->GetDiffuseTexture( CylinderDef.BitmapIndex );

    if( pDiffuse )
    {
        const render::primitive_draw_desc Material =
            fx_CreateMaterial( *pDiffuse, CylinderDef.CombineMode, CylinderDef.ReadZ,
