...
void fx_element::SubmitRender( const fx_effect_base* pEffect ) const
{
    (void)pEffect;
    ...
    if( FXDebug.ElementBounds )
        render::debug::Box( m_BBox, XCOLOR_GREEN );

    if( FXDebug.ElementCenter )
        render::debug::Marker( pEffect->GetL2W() * m_Translate, XCOLOR_GREEN );

    if( FXDebug.ElementAxis || FXDebug.ElementVolume )
    {
        const matrix4 localToWorld = GetL2W( pEffect );

        if( FXDebug.ElementAxis )
            render::debug::Axis( localToWorld, 3.0f );
        if( FXDebug.ElementVolume )
            render::debug::Box( bbox( vector3(-0.5f,-0.5f,-0.5f),
                             vector3( 0.5f, 0.5f, 0.5f) ), localToWorld, XCOLOR_BLUE );
    }
#endif // DEBUG_FX
}
