...
{
    X_PROFILE_SCOPE_CATEGORY( "Context", "fx_shockwave::AdvanceLogic" );
...
void fx_shockwave::SubmitRender( const fx_effect_base* pEffect ) const
{
    X_PROFILE_SCOPE_CATEGORY( "Context", "fx_shockwave::SubmitRender" );
    ...
    const texture* pDiffuse = pEffect->GetDiffuseTexture( ShockWaveDef.BitmapIndex );
    ...
    if( FXDebug.ElementWire )
    {
        ...
        VERIFY( fx_SubmitMesh( wireMaterial, L2W, m_pVertex, m_pUV, m_pColor, m_NVerts, m_pIndex,
                               m_NIndexes, FX_MESH_TRIANGLE_STRIP ) );
    }
#endif // DEBUG_FX
