...
{
    X_PROFILE_SCOPE_CATEGORY( "Context", "fx_sphere::AdvanceLogic" );
...
void fx_sphere::SubmitRender( const fx_effect_base* pEffect ) const
{
    X_PROFILE_SCOPE_CATEGORY( "Context", "fx_sphere::SubmitRender" );
    ...
    const texture* pDiffuse = pEffect->GetDiffuseTexture( SphereDef.BitmapIndex );

    if( pDiffuse )
    {
        const render::primitive_draw_desc Material =
            fx_CreateMaterial( *pDiffuse, SphereDef.CombineMode, SphereDef.ReadZ );
    ...
    if( FXDebug.ElementWire )
    {
        ...
        VERIFY( fx_SubmitMesh( wireMaterial, L2W, m_pVertex, m_pUV, m_pColor, m_NVerts, m_pSIndex,
                               m_NSIndexes, FX_MESH_TRIANGLE_STRIP ) );
    }
