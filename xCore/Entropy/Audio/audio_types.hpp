...
struct voice
{
    ...
    xbool                   UseReservedStream;      // Voice can use reserved audio stream slots.
...
struct element
{
         ...
         channel*                pChannel;               // Backend channel(s).
         uncompressed_parameters Params;                 // Parameters set in the audio packager
...
struct backend_channel_data
{
    u32                 hChannel;   // Backend channel handle.
    s32                 CurrentPosition;
    s32                 BasePosition;
    u32                 StartSerial;
    xbool               StartPending;
...
struct channel
{
    ...
    backend_channel_data       Backend;            // Backend specific data.
};
...
struct audio_stream
{
    stream_type            Type;                                   // Type of stream (free/mono/stereo).
    stream_state           State;                                  // Lifecycle state.
    ...
    xbool                  StreamDone;                             // Stream done?
    xbool                  UseReservedSlot;                        // Stream occupies reserved capacity.
    ...
    u32                    ARAMWriteBuffer;                        // Which aram buffer is the write buffer?
    u32                    DecodeWriteCursor;                      // Runtime decoded ring write cursor.
    u32                    DecodedFrames;                          // Exact PCM frames emitted by the decoder.
    u32                    DecodedEndFrame;                        // Exact PCM frame where runtime decode ended.
    audio_stream_decoder*  pDecoder;                               // Stream decoder instance.
};
