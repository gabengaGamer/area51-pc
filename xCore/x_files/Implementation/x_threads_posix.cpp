...
#error "This is only for POSIX targets. Please check build exclusion rules"
#endif
...
static 
xbool JoinThread( linux_thread_state& State )
{
    timespec Deadline;
...
#if defined(TARGET_ANDROID)
    // Bionic does not expose pthread_timedjoin_np. The worker signals this
    // semaphore on normal return and through pthread cleanup on pthread_exit.
    int WaitResult;
    do
    {
        WaitResult = sem_timedwait( &State.CompletionSemaphore, &Deadline );
    }
    ...
    return (pthread_join( State.Thread, NULL ) == 0);
#else
    return (pthread_timedjoin_np( State.Thread, NULL, &Deadline ) == 0);
#endif
}
...
static
void ThreadCompletionCleanup( void* pData )
{
    linux_thread_state* pState = (linux_thread_state*)pData;
    sem_post( &pState->CompletionSemaphore );
}
...
void* ThreadEntry( void* pData )
{
    ...
    pthread_cleanup_push( ThreadCompletionCleanup, pState );
                                    ...
                                    s32               InitialPriority )
{
    ...
    const xbool CompletionInitialized = (SuspendInitialized && (sem_init( &pState->CompletionSemaphore, 0, 0 ) == 0));
    ...
    if( CreateResult != 0 )
    {
        sem_destroy( &pState->CompletionSemaphore );
        sem_destroy( &pState->SuspendSemaphore );
        sem_destroy( &pState->StartSemaphore );
        std::free( pState );
        return Private;
...
void sys_thread_Destroy( xthread_private& Private )
{
    ...
    if( pState->OwnsNativeThread && !pthread_equal( pState->Thread, pthread_self() ) )
    {
        ...
        const xbool Joined = JoinThread( *pState );
        ASSERTS( Joined, "POSIX thread did not terminate before native join timeout." );
        if( Joined == FALSE )
        {
            std::abort();
        }
