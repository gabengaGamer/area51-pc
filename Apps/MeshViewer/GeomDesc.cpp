...
geom_rsc_desc::geom_rsc_desc( rsc_desc_type& Desc ) : rsc_desc( Desc )
{
    ...
    x_memset( m_PhysicsSource, 0, sizeof( m_PhysicsSource ) );
    x_memset( m_SettingsFile, 0 , sizeof( m_SettingsFile ) );
    x_memset( m_SelectedMesh, 0, sizeof(m_SelectedMesh) );
    m_SourceMeshList.Clear();
    m_SourceDiffuseList.Clear();
    m_UVAnimList.Clear();
}
...
void geom_rsc_desc::OnEnumProp( prop_enum& List )
{
    ...
    List.PropEnumFileName( "ResDesc\\FileName",
                           ...
                           "Select a geometry source file", 0 );

    // Physics file
    List.PropEnumFileName( "ResDesc\\PhysicsMatx",
                           "Geometry source files (*.*)|*.*||",
                           "Select a geometry source containing physics data", 0 );
    ...
    List.PropEnumButton( "ResDesc\\ForceMeshInfoUpdate",
                         "Reload the source file and update mesh, UV and texture information",
...
void geom_rsc_desc::OnEnumLODs( prop_enum& List, s32 VMeshId, s32 LodId )
{
    ...
    s32 i;
    if( m_SourceMeshList.GetCount() )
    {
        for( i = 0; i < m_SourceMeshList.GetCount(); i++ )
        {
            MeshEnumList += m_SourceMeshList[i].Name;
            MeshEnumList += "~";
        }
    }
    else
...
void geom_rsc_desc::OnEnumDiffTexture( prop_enum& List, s32 VTexId, s32 DiffId )
{
    ...
    if( DiffId == 0 )
    {
        ...
        s32 i;
        if( m_SourceDiffuseList.GetCount() )
        {
            for( i = 0; i < m_SourceDiffuseList.GetCount(); i++ )
            {
                TexEnumList += m_SourceDiffuseList[i].Name;
                TexEnumList += "~";
            }
        }
        else
...
xbool geom_rsc_desc::OnProperty( prop_query& I )
{
    ...
    else if( I.IsVar( "ResDesc\\PhysicsMatx" ) )
    {
        if( I.IsRead() )
        {
            I.SetVarFileName( m_PhysicsSource, MAX_NAME_LENGTH );
        }
        else
        {
            x_strsavecpy( m_PhysicsSource, I.GetVarFileName(), MAX_NAME_LENGTH );
        }
    }
...
{
    // Add main geometry source.
    List.Append() = m_FileName;
    
    // Add physics source if specified.
    if( m_PhysicsSource[0] )
        List.Append() = m_PhysicsSource;
...
void geom_rsc_desc::OnCheckIntegrity( void )
{
    ...
    if( m_FileName[0] == 0 )
        x_throw( "You must enter a geometry source file name" );
}
...
void geom_rsc_desc::RefreshSourceFileInfo( void )
{
    ...
    // Load through the registered source adapter.
    rawmesh2 RawMesh;
    xstring Error;
    xbool bLoaded = geom_source::LoadMesh( m_FileName, RawMesh, Error );
    if( bLoaded )
    {
        ...
        for( i = 0; i < RawMesh.m_nSubMeshs; i++ )
        {
            ...
            // of geometry's mesh
            source_mesh_info& MeshInfo = m_SourceMeshList.Append();
        ...
        for( i = 0; i < RawMesh.m_nMaterials; i++ )
        {
            ...
            if( iRawTex >= 0 )
            {
                ...
                for( j = 0; j < m_SourceDiffuseList.GetCount(); j++ )
                {
                    if( !x_stricmp( m_SourceDiffuseList[j].Name, FName ) )
                    {
                        bAlreadyAdded = TRUE;
                        break;
                    }
                ...
                if( !bAlreadyAdded )
                {
                    source_tex_info& TexInfo = m_SourceDiffuseList.Append();
