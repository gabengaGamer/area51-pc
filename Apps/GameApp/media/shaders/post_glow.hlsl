...
{
    float4 GlowParams0;   // x=cutoff, y=motion blend, z=accumulate weight, w=unused
    float4 GlowParams1;   // x=step.x, y=step.y, z=unused, w=unused
};

static const float kGlowIntensityScale = 6.0f;
...
float4 PS_Downsample( float4 Pos : SV_POSITION, float2 UV : TEXCOORD0 ) : SV_Target
{
    ...
    float  intensityScale = kGlowIntensityScale;
    ...
    for( int i = 0; i < 4; ++i )
    {
        ...
        float  intensity        = max( diff, 0.0f ) * blend;

        colorAccum += sample.rgb * intensity;
        weightAccum += intensity;
    }

    float glowAlpha = saturate( weightAccum * 0.25f );
    float3 glowColor = colorAccum * (intensityScale * 0.25f);
...
float4 SampleBlur( float2 UV, float2 Step )
{
    ...
    float4 color = GlowSource.SampleLevel( samPoint, UV, 0.0 ) * weights[0];
    ...
    for( int i = 1; i < 5; ++i )
    {
        ...
        color += (samplePos + sampleNeg) * weights[i];
    }
...
{
    float  blend  = saturate( GlowParams0.y );
