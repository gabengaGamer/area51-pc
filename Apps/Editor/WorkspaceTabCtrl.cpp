...
CEditorWorkspaceTabCtrl::CEditorWorkspaceTabCtrl()
{
...
BOOL CEditorWorkspaceTabCtrl::Create(CWnd* pParentWnd, UINT nID, LPCTSTR pWindowName, CSize SizeDefault, DWORD dwAlignStyle, DWORD dwStyle)
{
...
CFrameWnd* CEditorWorkspaceTabCtrl::CreateFrameDocView(CRuntimeClass* pFrameClass, CRuntimeClass* pViewClass, CDocument* pDocument)
{
    ASSERT(pFrameClass);
    ASSERT(pViewClass);
...
BOOL CEditorWorkspaceTabCtrl::AddControl(LPCTSTR pLabel, CWnd* pView)
{
    if (pView == NULL)
        return FALSE;
...
CWnd* CEditorWorkspaceTabCtrl::GetView(int Index) const
{
...
void CEditorWorkspaceTabCtrl::SetActiveView(int Index)
{
    if ((Index < 0) || (Index >= (int)m_Views.size()))
        return;
...
void CEditorWorkspaceTabCtrl::SetCaption(LPCTSTR pCaption)
{
...
void CEditorWorkspaceTabCtrl::SetTabImageList(CImageList* pImageList)
{
    m_TabCtrl.SetImageList(pImageList);
}

BOOL CEditorWorkspaceTabCtrl::EnableToolTips(BOOL bEnable)
{
    m_TabCtrl.EnableToolTips(bEnable);
    return TRUE;
}

void CEditorWorkspaceTabCtrl::OnTabSelChange(int nIDCtrl, CTabCtrl* pTabCtrl)
{
    UNREFERENCED_PARAMETER(nIDCtrl);
    UNREFERENCED_PARAMETER(pTabCtrl);
...
void CEditorWorkspaceTabCtrl::UpdateView(int Index, BOOL bActivate)
{
...
CWnd* CEditorWorkspaceTabCtrl::GetWorkspaceView(CRuntimeClass* pViewClass)
{
    CWnd* pWnd = NULL;
...
BOOL CEditorWorkspaceTabCtrl::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
...
LRESULT CEditorWorkspaceTabCtrl::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
...
CSize CEditorWorkspaceTabCtrl::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
...
void CEditorWorkspaceTabCtrl::LayoutChildren()
{
